...
1. **No user login / per-user UI flow**

   * README shows pages and routes but you said app “just starts with admin page”. The codebase lacks a proper multi-user auth flow and user login page; admin credentials default to `"admin"/"admin"` and may be hard-coded or not changeable in profile UI. This is a major security/usability gap.&#x20;

2. **Automation starts without validating Instagram login**

   * Scheduler/start endpoints appear to run tasks without strong per-account `is_logged_in()` gating. This risks running automations when session invalid or credentials gone, causing Instagram challenge/blocks. README mentions state checking, but we must enforce *all actions* check `bot.is_authenticated_for(user_id)` first.&#x20;

3. **No per-user automation instance / shared single bot engine**

   * README references integrated bot controller but it reads like a single-controller architecture. You requested every user have their own isolated automation instance; current architecture likely uses a shared `client` or shared scheduler which will cause state bleed, concurrency issues, and session conflicts.&#x20;

4. **Inconsistent credential handling & credential lifecycle**

   * README gives two descriptions: “No credential storage” and also an `instagram_credentials` table with encrypted password. There’s a possible mismatch between intended behavior and actual code. Need a clear lifecycle: (A) store encrypted creds only, (B) never log plaintext, (C) store derived session blobs for restoration.&#x20;
...
Below are prioritized tasks with code-level guidance, DB migrations, endpoints, and sample snippets. Implement in this order (1 → 6) to minimize breakage.

## Priority 1 — Enforce “automation paused until valid account logged in”

**Goal:** No automation task runs unless the target user has a valid, tested Instagram session.

### DB changes

Add `bot_instances` and `user_bot_status` (if not present):

```sql
-- migration: create table user_bot_status
CREATE TABLE user_bot_status (
...
* Wrap every action entry point with a decorator that checks `user_bot_status.session_valid` and `user_bot_status.bot_running` before enqueuing/starting tasks.
* Add an API: `POST /api/users/:id/test-session` to run `InstagramAuth.test_connection()` for that user's stored session; returns success/failure and sets `session_valid`.
...
def require_valid_session(func):
    ...
    def wrapper(user_id, *args, **kwargs):
        status = db.fetch_one("SELECT session_valid FROM user_bot_status WHERE user_id=%s", (user_id,))
        if not status or not status["session_valid"]:
            return {"success": False, "error": "SESSION_INVALID", "message": "Instagram session invalid. Please login and verify."}
        return func(user_id, *args, **kwargs)
    return wrapper
```

Use on all task routes:

```python
@app.post("/api/bot/actions/follow-hashtag")
@require_valid_session
def follow_hashtag(user_id): ...
```

### Scheduler

* Scheduler must query `user_bot_status` and only schedule tasks for users where `session_valid = true` and `bot_running = true`.
* Add function to re-validate sessions nightly and mark `session_valid=false` if tests fail.

---

## Priority 2 — Per-user automation instance architecture
...
1) Enforce automation paused until valid Instagram session:
   - Add table user_bot_status and bot_instances as per spec.
   - Add decorator require_valid_session to all /api/bot/actions/* endpoints.
   - Add API endpoint POST /api/users/:id/test-session that calls InstagramAuth.test_connection() and updates user_bot_status.session_valid.
   - Scheduler should only schedule tasks for users with session_valid=true and bot_running=true.

2) Implement per-user BotInstance architecture:
   - Add BotInstanceManager singleton and BotInstance class; create per-user sqlite files.
   - Update /api/bot/login to create/restore the BotInstance for that user, persist encrypted session blob, and set user_bot_status.
   - Update start/stop endpoints to start/stop that user's BotInstance scheduler.
